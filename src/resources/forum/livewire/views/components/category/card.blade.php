        ...
        @foreach ($category['children'] as $subcategory)
            <div class="flex mt-4">
                ...
                <div class="grow flex items-center justify-items-center bg-white shadow-md rounded-lg p-4">
                    ...
                    <div class="flex-1 text-right">
                        @if ($subcategory['accepts_threads'])
                            @if ($subcategory['newest_thread'])
                                <div>
                                    <a href="{{ $subcategory['newest_thread']['route'] }}">{{ $subcategory['newest_thread']['title'] }}</a>
                                    @include ('forum::components.timestamp', ['carbon' => new \Carbon\Carbon($subcategory['newest_thread']['created_at'])])
                                </div>
                            @endif
                            @if ($subcategory['latest_active_thread'] && $subcategory['latest_active_thread']['reply_count'] > 1)
                                <div>
                                    <a href="{{ Forum::route('thread.show', $subcategory['latest_active_thread']['last_post']) }}">Re: {{ $subcategory['latest_active_thread']['title'] }}</a>
                                    @include ('forum::components.timestamp', ['carbon' => new \Carbon\Carbon($subcategory['latest_active_thread']['last_post']['created_at'])])
                                </div>
                            @endif
                        @endif
                    </div>
